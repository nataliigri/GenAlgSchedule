...
class HtmlOutput:
    ...
    def generateTimeTable(solution, slot_table):
        ...
        for cc, reservation_index in items():
            ...
            # Перевіряємо наявність уже зарезервованого часу для цього періоду та кімнати
            ...
            # Якщо вже є заняття в цей період на той самий день, пропускаємо курс
            if room_duration[dayId] != 0:
                continue

            # Занотовуємо час заняття
            room_duration[dayId] = dur

            # Перевірка для наступних періодів, щоб уникнути перекриття
            for m in range(1, dur):
                next_key = (periodId + m, roomId)
                if next_key not in slot_table:
                    slot_table[next_key] = ROOM_COLUMN_NUMBER * [0]
                # Якщо в наступному періоді вже є курс, пропускаємо поточний курс
                if slot_table[next_key][dayId] < 1:
                    slot_table[next_key][dayId] = -1

            # Додаємо курс в розклад, якщо все ок
